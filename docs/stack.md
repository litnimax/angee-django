...
| Celery + Redis | Task transport, worker execution, retries, queue routing, and periodic dispatch | hosted by the `angee.tasks` framework app; Redis-backed broker; task bodies acquire Angee locks and delegate state changes to model/manager owners |
...
| mautrix[encryption] + python-olm | Matrix client protocol and E2EE — password login, sync, media negotiation, encrypted attachments/rooms, SSSS recovery keys, and cross-signing | `messaging_integrate_matrix` puppets one user's own Matrix account on the dedicated `matrix` queue and keeps mautrix's state/crypto stores under `ANGEE_DATA_DIR/matrix/<sqid>`. MPL-2.0. `matrix-nio` was rejected as stale and without current cross-signing support. mautrix still uses libolm through `python-olm`, whose vendored C++/CMake sdist builds on every platform (no wheel for the supported Python); the locked `[tool.uv.extra-build-variables]` entry owns that build — it pins clang (`CC`/`CXX`) for the clang-only `-fdelayed-template-parsing` flag and the `CMAKE_POLICY_VERSION_MINIMUM` floor CMake 4 needs, so clang + cmake are host build requirements. E2EE is mandatory and imports fail fast when its boundary is incomplete. |
| asyncpg + aiosqlite | mautrix persistent crypto-store database drivers | `PgCryptoStore` imports asyncpg for its shared store implementation, while the Matrix addon opens that store through mautrix's `sqlite://` database adapter backed by aiosqlite. Both are direct runtime dependencies so encrypted-room support cannot silently degrade. |
